...
export class BasicFormComponent extends BaseComponent implements TISCoreService {
  ...
  get appMeta(): Promise<TISBaseProfile> {
  ...
  template: `
    <div [ngClass]="{'sidebar': !tabBarExtraContent}">
      ...
      <div [ngClass]="{'float-right': !tabBarExtraContent}">
        <button *ngIf="!saveDisabled" nz-button nzType="primary" (click)="_saveClick()"><span nz-icon
                                                                                              nzType="save"
      ...
      .subscribe((params: Params) => {
        ...
        if (!this._getCurrentAppCache) {
          ...
          if (!appTisService.currentApp && collectionName) {
            appTisService.currentApp = new CurrentCollection(0, collectionName, AppType.DataX);
