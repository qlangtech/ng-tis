...
import {
  ...
  PluginType,
  SavePluginEvent,
  TisResponseResult, VerifyConfig
} from "./tis.plugin";
...
export class TargetPluginCfg {
  ...
  pluginType: PluginName = 'params-cfg';
  pluginDialogOpts: OpenPluginDialogOptions = null;
  pluginInitialParams: HttpParams = null;

  constructor(public targetDesc: string, pluginType?: PluginName) {
    if (pluginType) {
 ...
 * @param cpt
 */
export function openParamsCfg(targetDesc: TargetPluginCfg, appendExtraParam = '', drawerService: NzDrawerService, cpt: TISCoreService, dialogTitle?: string): Promise<PluginSaveResponse> {
  //console.log(targetDesc);
  let pluginMeta: PluginType = {
    "name": targetDesc.pluginType,
    "require": true,
    "extraParam": "targetItemDesc_" + targetDesc.targetDesc + (appendExtraParam ? "," + appendExtraParam : ''),
    "descFilter": {
      "localDescFilter": (desc: Descriptor) => {
        //  console.log(desc);
        return targetDesc.targetDesc === desc.displayName;
      }
    }
  };
  let targetPlugin: TargetPlugin = {
    hetero: targetDesc.pluginType,
    descName: targetDesc.targetDesc,
    pluginInitialParams: targetDesc.pluginInitialParams
  };
    ...
    ItemPropValComponent.checkAndInstallPlugin(drawerService, cpt, pluginMeta, targetPlugin)
      .then((desc) => {
        ...
        desc.forEach((d) => {
          ...
          let opts: OpenPluginDialogOptions = {shallLoadSavedItems: true};
          if (targetDesc.pluginDialogOpts) {
            opts = Object.assign(opts, targetDesc.pluginDialogOpts);
          }
          // console.log(opts);
          PluginsComponent.openPluginDialog(opts
            , cpt, d, pluginMeta, dialogTitle ? dialogTitle : targetDesc.targetDesc, (biz) => {
              resolve(biz);
            });
        });
      }, (rejectReason) => {
...
@Component({
  ...
  template: `
    <nz-spin [nzSpinning]="this.formDisabled" [nzDelay]="1000" nzSize="large">
      ...
      <ng-template #pluginForm let-h="h" let-index="index" let-pluginMeta="pluginMeta">
        ...
        <div *ngFor=" let item of h.items " style="position: relative"
          ...
          <div *ngIf="item.containAdvanceField" style="padding-left: 20px">
            <nz-switch class="advance-opts" nzSize="small" nzCheckedChildren="高级"
      ...
      <form nz-form [nzLayout]="compactVerticalLayout ? 'vertical' : 'horizontal'"
            [ngSwitch]="shallInitializePluginItems || useCollapsePanel">
...
export class PluginsComponent extends AppFormComponent implements AfterContentInit, AfterViewInit, OnDestroy {
  ...
  public static postHeteroList(basicModule: BasicFormComponent, pluginMetas: PluginType[], heteroList: HeteroList[]
    ...
    , errProcessCallback?: (r: TisResponseResult) => void): Promise<void> {
    ...
    if (savePluginEvent.skipPluginSave) {
      url += "&skipPluginSave=true"
    }
  ...
  configCheck(h: HeteroList, item: Item, event: MouseEvent) {
    ...
    let savePlugin = Object.assign(new SavePluginEvent(), this.savePluginEventCreator());
    savePlugin.verifyConfig = VerifyConfig.VERIFY;
  ...
  _deletePlugin(event: MouseEvent) {
    this.modalService.confirm({
      ...
      nzOnOk: () => {
        ...
        e.verifyConfig = VerifyConfig.FIRST_VALIDATE;
        if (!e.postPayload) {
          e.postPayload = {};
        }
...
//console.log(e);
  ...
  _savePluginInfo(event: MouseEvent, savePluginEvent: SavePluginEvent, pluginTypes: PluginType[], _heteroList: HeteroList[]) {
    ...
    let formContext: BasicFormComponent = this;
    ...
    PluginsComponent.addManipulate(this, pluginMeta, pluginDesc)
      .then((result) => {
        if (pluginDesc.manipulateStorable) {
          ...
          for (let mplugin of PluginManipulateMeta.wrapManipulateStore(result.biz())) {
            // item.dspt.manipulate.stored = [...item.dspt.manipulate.stored, new PluginManipulateMeta(pluginDesc, result.biz())]
            item.dspt.manipulate.stored = [...item.dspt.manipulate.stored, mplugin];
            this.cdr.detectChanges();
          }
