        ...
        <ng-container [ngSwitch]="compactVerticalLayout && !asyncHelp">
          ...
          <ng-container *ngSwitchCase="true">
            ...
            <ng-container *ngIf="descContent">
              <span class="inline-help-text"
                    [title]="!descContentShow ? descContent : ''">{{ truncatedHelpText }}</span>
        ...
        <ng-container [ngSwitch]="_pp.primaryVal">
          <ng-container *ngSwitchCase="true">
                 ...
                 <ng-container *ngSwitchCase="10">
                  ...
                  <ng-container *ngSwitchCase="12">
                      ...
                      <nz-input-number [attr.data-testid]="_pp.key" style="width: 50%;" [disabled]="disabled"
                                       *ngIf="_pp.primaryVal"
                                       [(ngModel)]="_pp.primary"
                                       [name]="_pp.key" (ngModelChange)="inputValChange(_pp,$event)"></nz-input-number><li
                    style="display: inline-block;margin-left: 3px;font-size: 12px"><nz-tag
                    nzColor="default">{{ _pp._eprops['unit'] }}</nz-tag></li>
                  </ng-container>
                  ...
                  <ng-container *ngSwitchCase="fieldTypeEnums">
                    <!--ENUM-->
                      <nz-select [attr.data-testid]="_pp.key" [nzMode]="_pp.enumMode" [disabled]="disabled"
                                 [(ngModel)]="_pp.primary" [name]="_pp.key"
                                 (ngModelChange)="inputValChange(_pp,$event)"
                                 [nzCustomTemplate]="enumOptsTpl"
                                 nzShowSearch nzAllowClear>
                           <nz-option nzCustomContent *ngFor="let e of _pp.getEProp('enum')" [nzLabel]="e.label"
                                      [nzValue]="e.val" [nzTitle]="e.endType?e.endType:null">
                             <span nz-icon class="icon-badge" style="font-size: 18px" *ngIf="e.endType"
                                   [nzType]="e.endType"
                                   nzTheme="fill"></span> {{ e.label }}</nz-option>
                       </nz-select>
                      <ng-template #enumOptsTpl let-opt> <span nz-icon class="icon-badge" *ngIf="opt.nzTitle"
                                                               [nzType]="opt.nzTitle"
                  ...
                  <ng-container *ngSwitchCase="6">
                    <!--select-->
                      ...
                      <nz-select [attr.data-testid]="_pp.key" [disabled]="disabled" [nzMode]="_pp.enumMode"
                       ...
                       <ng-template #selOptsTpl let-opt> <span nz-icon class="icon-badge" style="font-size: 18px"
                                                               *ngIf="opt.nzTitle"
                                                               [nzType]="opt.nzTitle"
                                                               nzTheme="fill"></span> {{ opt.nzLabel }}</ng-template>
                  </ng-container>
                    ...
                    <ng-container [ngSwitch]="_pp.tuplesViewType">
                     ...
                     <ng-container *ngSwitchCase="'jdbcTypeProps'">
                      ...
                      <ng-container *ngSwitchCase="'tableJoinFilterCondition'">
                       ...
                       <ng-container *ngSwitchCase="'multiSelectSingleVal'">
                         <multi-select-single-val [attr.data-testid]="_pp.key" [tabletView]="_pp.mcolsEnums"
                                                  [error]="_pp.error"/>
                      </ng-container>
            ...
            <ng-container *ngIf="this.createRouter && !this.disabled">
              {{ createRouter.assistType }}
              <button [attr.data-testid]="_pp.key+'_create_router'" class="assist-btn" nz-button
                      nz-dropdown
...
export class ItemPropValComponent extends BasicFormComponent implements AfterContentInit {
  ...
  @Output()
  valChange: EventEmitter<ItemPropVal> = new EventEmitter<ItemPropVal>();
      ...
      .then((desc) => {
        ...
        desc.forEach((d) => {
          ...
          let pluginTp: PluginType = {
            ...
            require: true,
            extraParam: "append_true,targetItemDesc_" + (targetPlugin.targetItemDesc || d.displayName)
          };
  ...
  inputValChange(_pp: ItemPropVal, $event: Event) {
    //console.log([$event, _pp]);
    delete _pp._error;
    this.valChange.emit(_pp);
    // console.log("inputValChange");
    // $event.stopPropagation();

    // let evtChange = _pp.getEProp("event_change");
    // if (evtChange) {
    //      eval(evtChange)
    // }
  }
