...
export class ItemPropVal extends ErrorFeedback {
  ...
  public setPropValEnums(cols: Array<{
    name: string,
    value: string,
    checked?: boolean
  }>, colItemChecked?: (optVal) => boolean) {
    ...
    cols.forEach((s) => {
      enums.push({label: s.name, val: s.value, checked: s.checked || colItemChecked(s.value)})
    });
    //console.log([enums,cols])
...
export class Descriptor {
  ...
  public static wrapDescriptors(descriptors: Map<string /* impl */, Descriptor>)
    ...
    for (let impl in descriptors) {
      ...
      let stepsCfg = null;
      /**
       * 对应服务端DescriptorsJSON.multiStepsPropsSet() 方法
       */
...
export interface TisErorsResult {
  errormsg?: string[];
  errorfields?: Array<Array<Array<IFieldError> | Map<string, Array<IFieldError>>>>;
}

export interface TisResponseResult extends TisErorsResult {
  bizresult?: any;
  success: boolean;
...
export type ColsAndMeta = {
  colMetas: Array<DataTypeMeta>,
  tabMapper: { sourceCols: Array<ReaderColMeta>, to: string, from: string }
}
  ...
  public static buildMultiSelectedAttr(val: Array<any>, at: AttrDesc, newVal: ItemPropVal) {
    ...
    if (mcols = enumVal["tabMapper"]) {
      ...
      switch (contentType) {
        ...
        case TuplesPropertyType.TableJoinMatchCondition: {
      ...
      let selectableCol: Array<{ val: string, label: string, checked?: boolean }> = at.eprops[KEY_OPTIONS_ENUM];
      //console.log(selectableCol);
      if (!selectableCol) {
        throw new Error("selectableCol can not be null");
      }
      let cols: Array<{ name: string, value: string, checked?: boolean }> = null;
