...
import {HeteroList, Item, ItemPropVal, SuccessAddedDBTabs, TisResponseResult} from "../common/tis.plugin";
...
export enum ExecAddModel {
    /**
     * 针对dataSource的 添加表执行模式
     */
    DataSource = 0,
    /**
     * 针对Pipeline的 添加表执行模式
...
export class TableAddComponent extends BasicFormComponent implements OnInit {
    ...
    @Input() processMode: {
        ...
        dbId?: string,
        dbName?: string,
        // dataXPipelineName?: string,
        dtoSet?: (dto: DataxDTO) => any,
        isNew: boolean
    } = {isNew: true};

    @Input() execAddMode: ExecAddModel = ExecAddModel.DataSource;
    ...
    ngOnInit(): void {
        ...
        switch (this.execAddMode) {
            case ExecAddModel.DataSource: {
            ...
            case ExecAddModel.Pipeline: {
                configFST.set(DataxAddStep4Component, {next: null, pre: null});
                break;
            }
            default:
                throw new Error();
        ...
        dataXDto.componentCallback.step4.subscribe((cpt) => {
            ...
            cpt.nextStep.subscribe(() => {
                ...
                // console.log(  cpt.subFormHetero.items );
                let hasSetItem = false;
                // let selectedTabCount =0;
                cpt.subFormHetero.items.forEach((item: Item) => {
                    this.activeModal.close(new SuccessAddedDBTabs(dataXDto.tablePojo, <{
                        [key: string]: Array<Item>
                    }>item.vals));
                    // selectedTabCount = item.vals.
                    hasSetItem = true;
                })
                ...
                if (dataXDto.tablePojo) {
                    this.successNotify("数据源:" + dataXDto.tablePojo.dbName + "已完成表导入设置");
                } else {
                    this.successNotify("已完成目标表设置");
                }
            })
        });
        let mode = this.processMode;

        if (this.processMode.dtoSet) {
            this.processMode.dtoSet(dataXDto);
        }
        //console.log([this.execAddMode, dataXDto]);
        switch (this.execAddMode) {
            case ExecAddModel.Pipeline: {
                // 在AI Agent模式下执行
                this.multiViewDAG.loadComponent(DataxAddStep4Component, dataXDto);
                break;
            }
            case ExecAddModel.DataSource: {
                ...
                TableAddStep1Component.getReaderDescAndMeta(this, dataXDto)
                    .then((r) => {
                        if (r.success) {
        ...
        if (this.execAddMode === ExecAddModel.DataSource) {
