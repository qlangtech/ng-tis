...
import {ExecAddModel, TableAddComponent} from "./table.add.component";
...
//    addDb.setPluginMeta([{name: 'datasource', require: true, extraParam: `dsname_${this.selectedDb.dbName},update_true,type_${facade ? "facade" : db_model_detailed}`}])
    ...
    // public static openAddTableDialogXX(cpt: BasicFormComponent, propSetter: (cpt: TableAddComponent)=>{}): Promise<SuccessAddedDBTabs> {
    //
    //
    //     return null;
    // }

    public static openAddTableForDataSourceDialog(module: BasicFormComponent, db: DataBase): Promise<SuccessAddedDBTabs> {
        return DatasourceComponent.openAddTableDialog(module,(cpt)=>{
            cpt.execAddMode = ExecAddModel.DataSource;
            let pm = cpt.processMode;
            if (db) {
                pm.dbId = db.dbId;
                pm.dbName = db.dbName;
            }
     ...
     * @param propSetter
     */
    public static openAddTableDialog(cpt: BasicFormComponent, propSetter: (cpt: TableAddComponent) => any): Promise<SuccessAddedDBTabs> {

        let modalRef = cpt.openDialog(TableAddComponent, {nzTitle: "添加表", nzWidth: "1000px"});
        let tableAddCpt: TableAddComponent = modalRef.getContentComponent();

        propSetter(tableAddCpt);
    ...
    public addTableBtnClick(): void {
        ...
        if (this.selectedDb && this.selectedDb.dbId) {
            database = new DataBase(this.selectedDb.dbId, this.selectedDb.dbName);

        } else if (this.selectedTable && this.selectedTable.tableName) {
            // pm.dbId = ;
            // pm.dbName = ;
            database = new DataBase(`${this.selectedTable.dbId}`, this.selectedTable.dbName);
        }


        DatasourceComponent.openAddTableForDataSourceDialog(this, database).then((r: SuccessAddedDBTabs) => {
            // 添加表成功
