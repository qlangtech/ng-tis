...
export class CompletionService extends MessageListenersManager {
  ...
  private receivers = new WeakMap<ITextModel, string>();
  private bound = false;
  ...
  registerAsCompletionReceiver(model: ITextModel, pid: string): void {
  ...
  unregister(model: ITextModel): void {
  ...
  private getIdForModel(model?: ITextModel): string | null {
